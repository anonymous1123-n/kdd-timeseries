...
def make_beta_schedule(schedule="linear", num_timesteps=100, start=1e-5, end=1e-2):
    if schedule == "linear":
...
class DiffusionEmbedding(nn.Module):
    def __init__(self, num_steps, embedding_dim=128, projection_dim=None): #128 iTransformer 64
        super().__init__()
...
class CNN_DiffusionUnet(nn.Module):
    ...
    def __init__(self, args, num_vars, seq_len, pred_len, diff_steps):
        ...
        kernel_size = 3
        padding = 1
        self.channels = args.ddpm_inp_embed # ddpm_inp_embed : 256 => 2048
        
        self.input_projection = InputConvNetwork(args, self.num_vars, self.channels, num_layers=args.ddpm_layers_inp)

        self.dim_diff_steps = args.ddpm_dim_diff_steps # 256 => 2048
        
        self.diffusion_embedding = DiffusionEmbedding( # 2. sinusoidal position embedding
        ...
        self.enc_conv = InputConvNetwork(args, self.channels+self.dim_diff_steps, self.dim_intermediate_enc, num_layers=args.ddpm_layers_I) # 3. Encoder

        self.cond_projections = nn.ModuleList() # 4. Condition Projection
        ...
        if args.ablation_study_F_type == "Linear":
            ...
            self.cnn_cond_projections_test = InputConvNetwork(args, self.num_vars, self.pred_len+self.label_len, num_layers=args.cond_ddpm_num_layers, ddpm_channels_conv=args.cond_ddpm_channels_conv)
    ...
    def forward(self,  yn=None, diffusion_step=None, cond_info=None, y_clean=None):
        ...
        x = yn # y_t_batch : torch.Size([32, 240, 7])
        ...
        #     pred_out = torch.zeros([yn.size(0),self.num_vars,self.pred_len],dtype=yn.dtype).to(yn.device) # pred_out torch.Size([800, 7, 192])
        # else:
        #     pred_out = torch.zeros([yn.size(0),self.num_vars,self.pred_len+self.label_len],dtype=yn.dtype).to(yn.device)
        ...
        #     else:
        #         temp_out = self.cond_projections[i](cond_info[:, i, :self.ori_seq_len])
        #         pred_out[:, i, :self.pred_len] = temp_out  # First self.pred_len elements
        #         pred_out[:, i, self.pred_len:] = temp_out[:, :self.label_len]  # Extend to match pred_len + label_len
        ...
        #     else:
        #         temp_out = self.cnn_cond_projections_test(cond_info[:,:,:self.ori_seq_len])
        #         pred_out += self.cnn_linear(temp_out).permute(0,2,1)
        ...
        # else:
        #     inp = torch.cat([x, pred_out, cond_info[:,:,self.ori_seq_len-self.label_len:]], dim=1)
        ...
        '''
        (train)
        x torch.Size([32, 256, 192])
        pred_out torch.Size([32, 7, 192])
        cond_info torch.Size([32, 7, 192])

        (test)
        x torch.Size([800, 256, 240])
        pred_out torch.Size([800, 7, 240])
        cond_info torch.Size([800, 7, 240])
        '''        
