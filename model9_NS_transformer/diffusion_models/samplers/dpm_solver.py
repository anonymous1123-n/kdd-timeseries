...
class NoiseScheduleVP:
    ...
    def marginal_alpha(self, t): 
        """
        Compute alpha_t of a given continuous-time label t in [0, T]. gamma_0에 해당함.
        """
        return torch.exp(self.marginal_log_mean_coeff(t))

    def marginal_std(self, t): 
        """
        Compute sigma_t of a given continuous-time label t in [0, T]. gamma_1에 해당함.
        """
        return torch.sqrt(1. - torch.exp(2. * self.marginal_log_mean_coeff(t)))
    
    def marginal_gamma2(self, t):
        """
        Compute gamma_2 of a given continuous-time label t in [0,T]
        """
        alpha_t = self.marginal_alpha(t)
        sigma_t = self.marginal_std(t)
        return 1 + (alpha_t-1)*(sigma_t+alpha_t)
...
class DPM_Solver:
    ...
    def data_prediction_fn(self, x, t):
        ...
        dims = x.dim()
        alpha_t, sigma_t = self.noise_schedule.marginal_alpha(t), self.noise_schedule.marginal_std(t) # alpha_t==gamma_0, sigma_t==gamma_1
        gamma_2 = self.noise_schedule.marginal_gamma2(t)
    ...
    def dpm_solver_first_update(self, x, s, t, model_s=None, return_intermediate=False):
        ...
        if self.predict_x0:
            ...
            x_t = (
                ...
                - expand_dims(sigma_t * phi_1, dims) * model_s # 여기가 논문에서 주장하는 denoising 내용
            )
            if return_intermediate:
                return x_t, {'model_s': model_s}
            else:
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=3, skip_type='time_uniform',              # 원래는 order 3
