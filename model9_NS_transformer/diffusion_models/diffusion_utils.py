...
# def p_sample(model, y_recon, x, x_mark, y, y_0_hat, y_T_mean, t, alphas, one_minus_alphas_bar_sqrt):
#     """
...
#     eps_theta = model(x, x_mark, 0, y, y_0_hat, t).to(device).detach() #eps_theta torch.Size([800, 240, 7])

#     # 여기서부터 수정
    
#     future_mixup = y_recon.to(device) # future_mixup torch.Size([800, 7, 240]) , y torch.Size([800, 240, 7])
#     future_mixup = future_mixup.permute(0,2,1)
#     eps_theta_correted = eps_theta + (future_mixup - y)*0.5
#     # y_0 reparameterization
#     y_0_reparam = 1 / sqrt_alpha_bar_t * (
#             y - (1 - sqrt_alpha_bar_t) * y_T_mean - eps_theta_correted * sqrt_one_minus_alpha_bar_t)
#     # posterior mean
...
# Reverse function -- sample y_0 given y_1
# def p_sample_t_1to0(model, y_recon, x, x_mark, y, y_0_hat, y_T_mean, one_minus_alphas_bar_sqrt):
#     device = next(model.parameters()).device
#     t = torch.tensor([0]).to(device)  # corresponding to timestep 1 (i.e., t=1 in diffusion models)
#     sqrt_one_minus_alpha_bar_t = extract(one_minus_alphas_bar_sqrt, t, y)
#     sqrt_alpha_bar_t = (1 - sqrt_one_minus_alpha_bar_t.square()).sqrt()
...
# def p_sample_loop(model, u_net, x, x_mark, y_0_hat, y_T_mean, n_steps, alphas, one_minus_alphas_bar_sqrt,cond_ts):
#     device = next(model.parameters()).device
#     z = torch.randn_like(y_T_mean).to(device)
#     cur_y = z + y_T_mean  # sample y_T
#     y_p_seq = [cur_y]
#     for t in reversed(range(1, n_steps)):  # t from T to 2 # simplified의 sample에 해당하는 부분
#         y_t = cur_y
#         model_in = z.permute(0,2,1)
#         t_tensor = torch.full((z.size(0),), t, dtype=torch.long, device=device)
#         model_out = u_net(model_in,t_tensor,cond_ts, y_clean=None).detach()
#         y_recon = model_out
#         cur_y = p_sample(model, y_recon, x, x_mark, y_t, y_0_hat, y_T_mean, t, alphas, one_minus_alphas_bar_sqrt)  # y_{t-1}
#         y_p_seq.append(cur_y)
#     assert len(y_p_seq) == n_steps
#     y_0 = p_sample_t_1to0(model, y_recon, x, x_mark, y_p_seq[-1], y_0_hat, y_T_mean, one_minus_alphas_bar_sqrt)
#     y_p_seq.append(y_0)
#     return y_p_seq
